        ...
        if state.board.squares[i].owner is state.currentPlayer:
            ...
            if state.currentPlayer is state.players[0]:
                ...
                if state.board.squares[i - 3].owner is not state.currentPlayer and state.board.squares[i - 3].owner is not None:
                    listOfMoves.append(Action(state.board.squares[i], state.board.squares[i-3], state.currentPlayer))
                    moves += 1
                    # Skip move
                    jumpTo = i
                    while True:
                        jumpTo -= 3
                        if jumpTo < 0:
                            listOfMoves.append(Action(state.board.squares[i], state.board.squares[12], state.currentPlayer))
                            moves += 1
                            break
                        if state.board.squares[jumpTo].owner is state.currentPlayer:
                            break
                        if state.board.squares[jumpTo].owner is None:
                            listOfMoves.append(Action(state.board.squares[i], state.board.squares[jumpTo], state.currentPlayer))
                            moves += 1
                            break

                # Forward Right and Left move
                if (i % 3 == 1 or i % 3 == 0) and state.board.squares[i - 2].owner is None:
                    listOfMoves.append(Action(state.board.squares[i], state.board.squares[i - 2], state.currentPlayer))
                    moves += 1
                if (i % 3 == 1 or i % 3 == 2) and state.board.squares[i - 4].owner is None:
                    listOfMoves.append(Action(state.board.squares[i], state.board.squares[i - 4], state.currentPlayer))
                    moves += 1
            ...
            # Reds turn
                ...
                if state.board.squares[i + 3].owner is not state.currentPlayer and state.board.squares[i + 3].owner is not None:
                    ...
                    while True:
                        jumpTo += 3
                        if jumpTo > 11:
                            listOfMoves.append(Action(state.board.squares[i], state.board.squares[13], state.currentPlayer))
                            moves += 1
                            break
                        if state.board.squares[jumpTo].owner is state.currentPlayer:
                            break
        ...
        if state.currentPlayer is state.players[0]:
            for i in range(3):
                if state.board.squares[i + 9].owner is None:
                    listOfMoves.append(Action(state.board.squares[13], state.board.squares[i + 9], state.currentPlayer))
                    moves += 1
        else:
            for i in range(3):
                if state.board.squares[i].owner is None:
                    listOfMoves.append(Action(state.board.squares[12], state.board.squares[i], state.currentPlayer))
                    moves += 1
    return listOfMoves
